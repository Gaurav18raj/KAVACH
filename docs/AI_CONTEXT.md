...
- **Key Directories:**
  ...
  - `backend/models.py` & `schemas.py`: SQLAlchemy database models (`User`, `BehavioralBaseline`, `TransactionHistory`) and Pydantic validation schemas.
...
1. **Behavioral Biometrics (Keystroke Dynamics):**
   ...
   - *Logic:* Handled by `behavioral_scorer.py`. Uses Z-Scores ($Z = |X - \mu| / \sigma$) to check if the current typing rhythm deviates significantly from the user's historical `BehavioralBaseline`.
